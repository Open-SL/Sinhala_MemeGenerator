...
from converter import Translate
...
def make_meme(topString, bottomString, filename):
	...
	imageSize = img.size
	topString = topString.decode('utf-8')
	bottomString = bottomString.decode('utf-8')

	# find biggest font size that works
	fontSize = int(imageSize[1]/5)
	font = ImageFont.truetype("./Sinhala Sangam MN.ttc", fontSize)
...
def convertSentence(somedata,type_):
	flag = False
	data = ""
	finString = ''
	
	for index, c in enumerate(somedata):
		if c == "(":
			flag = True
			continue
		elif c== ")":
			flag = False
			finString += Translate(data)
			data = ""
			continue
		if flag:
			data += c
...
def get_upper(somedata):
	...
	'''
	return convertSentence(somedata,'upper')

def get_lower(somedata):
	'''
	Handle Python 2/3 differences in argv encoding
	'''
	return convertSentence(somedata,'lower')
